...
const StockList = ({ stocks, stockTradeAllocations, priceHistory, 
  indexHistory, stockThesisData = [], holdingOrders, stockInstruments,
  ...
  const handleSaveThesis = async (stockObj, updatedThesis) => {
    ...
    // Resolve the thesis record id from thesis data (stockscurrentdata now holds
    // thesis only — no price/scrip). Persist via the thesis endpoint.
    const selectedStock = stockThesisData.find(rec => rec.name === stockName);
    if (!selectedStock) {
      console.error("No thesis record found for", stockName, "- cannot save thesis.");
      return;
    }
    ...
    // ✅ Update local merged view immutably so the UI reflects the save
    ...
    // ✅ Notify parent so the canonical thesis state stays in sync
    if (onThesisSaved) onThesisSaved(stockID, updatedThesis);
  };
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Thesis records keyed by stock name for O(1) lookup during merge/display.
  // Thesis is the only field still sourced from stockscurrentdata; price comes
  // from history and name/symbol from the instruments map.
  const thesisByName = useMemo(() => {
    const map = {};
  ...
  // --------------------------------------------------------------------------
  // Function to merge derived price (from history) + thesis (from thesisByName)
  // into the aggregated stock rows whenever inputs change.
  useEffect(() => {
    ...
    if (Object.keys(aggregatedStocks).length > 0) {
      const mergedData = Object.values(aggregatedStocks).map((stock) => {
        ...
        // Fullname always from the instruments map (single source for metadata)
        updatedStock.fullname = stockFullname || stock.name;

        // Thesis from thesisByName (stockscurrentdata → thesis only), independent
        // of price availability so it renders even when price history is missing.
        const thesisRec = thesisByName[stock.name];
        updatedStock.thesis_markdown = thesisRec?.thesis_markdown || "";
        updatedStock.thesis_last_updated = thesisRec?.thesis_last_updated
          ? new Date(thesisRec.thesis_last_updated).toISOString()
          : null;

        if (currentData) {
          // Access the current stock price (derived from price history)
          ...
          // Update current price and quote date (price map holds { price, date })
  ...
  }, [aggregatedStocks, latestPriceMap, stockInstruments, thesisByName]);
  // --------------------------------------------------------------------------
  ...
  }, [mergedStockData, previousPriceMap]);
  // --------------------------------------------------------------------------
