...
//
// CRITICAL PAYLOAD NOTE (root cause of the earlier "only Nifty 50 works" bug):
//   `cinfo` MUST be a SINGLE-QUOTED pseudo-JSON string, e.g.
//       {'name':'NIFTY 50','startDate':'01-Jul-2026',...}
//   This is the ASP.NET ScriptService convention — NOT standard JSON.
//   JSON.stringify() emits DOUBLE quotes, which the server silently rejects
//   (empty body). Midcap 150 / Smallcap 250 then returned nothing; Nifty 50
//   only appeared to work because it fell through to the Yahoo fallback.
//
...
//   and then never sends a body (client read-timeout). Always send a UA that
//   starts with "Mozilla/5.0".
//
// PUBLIC API:
...
// Prime the ASP.NET session cookie. Not strictly required (cold POST works),
// but defensive against Akamai bot sampling on some requests.
...
async function niftyPost(nseName, fromIso, toIso, cookie) {
  // CRITICAL: single-quoted pseudo-JSON (ASP.NET ScriptService), NOT JSON.
  // JSON.stringify would emit double quotes and the server returns empty.
  const cinfo =
    `{'name':'${nseName}',` +
