...
// Index history is delegated to a dedicated service (niftyindices origin +
// Yahoo fallback for Nifty 50). See services/indexService.js for rationale.
const { INDICES, fetchIndexHistory } = require("../services/indexService");
...
//
// INDICES: handled separately by services/indexService.js — NSE index history
// is NOT on nseindia.com's equity path; its origin is niftyindices.com.
// ============================================================================
...
// INDICES API
// ----------------------------------------------------------------------------
// SOURCE: niftyindices.com (origin of all Nifty index history) with a Yahoo
// fallback for Nifty 50 only. All fetch/parse/pacing logic lives in
// services/indexService.js — this route only orchestrates DB writes.
//
// INDICES config (DB symbol -> NSE name) is imported from the same service so
// there is a single source of truth shared by fetch + this route.
// ----------------------------------------------------------------------------
router.post("/update_indices", async (req, res) => {
  try {
    ...
    // -------------------------
    // Fetch + collect all rows (niftyindices primary; Yahoo fallback for N50)
    // -------------------------
    const allRows = [];
    for (const { symbol, from, to } of ranges) {
      const rows = await fetchIndexHistory(symbol, from, to); // [] if unknown symbol
      rows.forEach((r) => allRows.push({ symbol, date: r.date, close: r.close }));
    }
    if (allRows.length === 0) {
...
// Default symbol set is derived from the shared INDICES config.
router.get("/get_indices", async (req, res) => {
  try {
    const symbols = req.query.symbols
      ? req.query.symbols.split(",")
      : INDICES.map((i) => i.symbol); // ["NIFTY50","MIDCAP150","SMALLCAP250"]
