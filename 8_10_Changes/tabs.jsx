...
const Tabs = () => {
  ...
  const updateScripCodes = async (stockName, newScripCodes) => {
    // ✅ Check if stockName exists in currentStockData
    const existingStock = currentStockData.find(stock => stock.name === stockName);
  ...
  const handleCurrentStockUpdate = (updatedData) => {
    ...
    let stockToUpdate = [];
    // Update the stock price data in the currentStockData
    const newStockCurrentData = currentStockData.map((stockData) => {
    ...
    // Update the stock data in the currentStockData
    setCurrentStockData(newStockCurrentData);
  ...
  const getRemainingQty = (buyId, stocks, allocations) => {
    ...
    // FIX #10 — defensive parse of buy.quantity (latent: schema is INTEGER, but a
    // string-typed qty would coerce `str - num` to NaN and silently poison FIFO).
    return (parseFloat(buy.quantity) || 0) - soldQty;
  };
  // --------------------------------------------------------------------------
