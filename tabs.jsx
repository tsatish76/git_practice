...
const Tabs = () => {
  ...
  // --------------------------------------------------------------------------
  // FIX #6 — Two distinct shapes were conflated under one name `currentStockData`.
  //
  //   latestStockPriceMap : { [symbol]: { price, date } }   (memo, price merging)
  //   stockCurrentData    : [{ id, name, price, nse_scrip_code, bse_scrip_code,
  //                            thesis_markdown, ... }]        (array, current-data
  //                            records backed by /stockscurrentdata)
  //
  // The scrip-code / current-data / thesis handlers require the ARRAY shape
  // (.find/.map by name, ids for persistence). Passing them the memoized MAP
  // caused runtime crashes (map.find/map.map not a function + a nonexistent
  // setCurrentStockData). This mirrors the MF side (latestNAVMap + currentMFData).
  // --------------------------------------------------------------------------
  ...
  const latestStockPriceMap = useMemo(() => {
  ...
  // Current-data records (scrip codes, thesis, last quote) — array source of
  // truth for scrip/thesis handlers. Mirrors currentMFData on the MF side.
  ...
  // --------------------------------------------------------------------------
  const updateScripCodes = async (stockName, newScripCodes) => {
    // ✅ Check if stockName exists in stockCurrentData (array of records)
    const existingStock = stockCurrentData.find(stock => stock.name === stockName);

    const updatedCurrentStock = {
      ...(existingStock || {}), // Keep existing data if available
      name: stockName,
      price: isNaN(parseFloat(existingStock?.price)) ? 0.0 : parseFloat(existingStock.price), // Ensure price is a float
      nse_scrip_code: newScripCodes.nse_scrip_code,
      bse_scrip_code: newScripCodes.bse_scrip_code,
      // Store empty string date if it does not exists
      date: existingStock?.date && !isNaN(Date.parse(existingStock.date))
        ? new Date(existingStock.date).toISOString().split("T")[0]
        : null, // Use null for invalid or missing dates
    };

    const API_name = existingStock ? "PUT" : "POST";
    const payload = [updatedCurrentStock,];

    // Edit entries from database
    // use .then to call async function
    updateCurrentStockData(payload, API_name)
      .then((success) => {
        if (success) {
      ...
      .catch((error) => {
        console.error("🚨 Error in put CurrentStockData:", error);
      });

    // ✅ **Update React State**
    setStockCurrentData((prevData) => {
      if (existingStock) {
        // Update existing stock
        return prevData.map((stock) =>
          stock.name === existingStock.name
            ? {
              ...stock,
              nse_scrip_code: newScripCodes.nse_scrip_code,
              bse_scrip_code: newScripCodes.bse_scrip_code
        ...
        // Add new stock
        return [...prevData, updatedCurrentStock];
      };
    });
  };
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  const updateCurrentStockData = async (payload, API_name) => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(`${API_name}: Failed to add scrip code for Stock!`);
        const errorText = await response.text(); // Log server's response
        console.error(`Backend Error (${API_name}):`, errorText);
        return false;
      } else {
        return true;
      };
  ...
  // --------------------------------------------------------------------------
  // Function to update the current stock price data.
  const handleCurrentStockUpdate = (updatedData) => {

    let stockToUpdate = [];
    // Update the stock price data in stockCurrentData (array of records)
    const newStockCurrentData = stockCurrentData.map((stockData) => {
      const currData = structuredClone(stockData);  // create a deep copy
      const stock_name = stockData.name;

      const newStock = updatedData.find(stock => stock.name === stock_name);
      if (newStock) {

        let newDate = new Date(newStock.date);

        currData.fullname = newStock.fullname ?? "";
        currData.price = newStock.price ?? currData.price;
        currData.marketcap = newStock.marketcap ?? currData.marketcap ?? 0;
        currData.date = newDate.toISOString().split('T')[0]; // Keep YYYY-MM-DD format
        currData.time = newStock.time ?? currData.time ?? "";
        currData.exchange = newStock.exchange ?? currData.exchange ?? "";

        // Update
        stockToUpdate.push(currData);

      };
      return currData;
    });

    // Update the stock data in stockCurrentData
    setStockCurrentData(newStockCurrentData);

    // Update the data in the database
    updateCurrentStockData(stockToUpdate, "PUT");
  ...
  // --------------------------------------------------------------------------
  // FIX #6 — Fetch current-data records (scrip codes, thesis, last quote) into
  // the array state. Mirrors fetchMutualFundsCurrentData on the MF side.
  // NOTE: assumes GET /stockscurrentdata returns full records
  // [{ id, name, price, nse_scrip_code, bse_scrip_code, thesis_markdown, ... }]
  // (symmetric with GET /mutualfundscurrentdata). Adjust the endpoint/shape if
  // the backend differs.
  const fetchStockCurrentData = async () => {
    try {
      const response = await fetch(`${BASE_URL}/stockscurrentdata`);
      const data = await response.json();
      setStockCurrentData(Array.isArray(data) ? data : []);
    } catch (error) {
  ...
  const fetchAllInitialData = async () => {
    ...
    try {
      await Promise.all([
        ...
        fetchStockCurrentData(),        // FIX #6 — load current-data array
  ...
  // --------------------------------------------------------------------------
  // FIX #5 — Same-day BUY→SELL must be eligible for FIFO allocation.
  // Root cause: comparing full Date objects (new Date(s.date) <= new Date(sell))
  // fails when a BUY row's stored `date` carries a time component — its timestamp
  // exceeds the sell date's midnight and the same calendar day is wrongly
  // excluded, throwing "Insufficient BUY shares". Compare normalized YYYY-MM-DD
  // date keys instead so calendar-day equality is honoured deterministically.
  const toDateKey = (d) => {
    if (!d) return "";
  ...
  const buildFifoAllocations = (stocks, sellTrade) => {
    const sellKey = toDateKey(sellTrade.date);
    const availableBuys = stocks
      .filter(s =>
        s.symbol === sellTrade.symbol &&
        s.order_type === "BUY" &&
        toDateKey(s.date) <= sellKey
      )
        ...
        >
          {activeTab === "summary" ? (
            <SummaryTab
              ...
              currentStockData={latestStockPriceMap}
            ...
            <StockList
              ...
              currentStockData={stockCurrentData}
              holdingOrders={stockHoldingOrders}
              stockInstruments={stockInstruments}
              onOrdersEdit={handleStockOrderEdit}
              onScripCodesUpdate={updateScripCodes}
              onUpdateCurrentStockData={handleCurrentStockUpdate}
